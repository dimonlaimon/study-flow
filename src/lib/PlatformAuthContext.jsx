  ...
  useEffect(() => {
    ...
    async function initTelegram() {
      try {
        ...
        if (existing) {
          await supabase.from('profiles').update({ full_name, photo_url }).eq('telegram_id', telegram_id);
        } else {
          await supabase.from('profiles').insert({ telegram_id, full_name, photo_url, role: 'user' });
        }
        if (!active) return;

...
export default function DeadlineCard({ deadline, canDelete, onDelete }) {
    ...
    <div className={`rounded-xl border p-4 transition-all ${
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          ...
          {deadline.subject && (
            <p className="text-xs text-muted-foreground mb-1">{deadline.subject}</p>
          )}
        ...
        <div className="text-right shrink-0">
          ...
          <p className="text-xs text-muted-foreground">{format(dueDate, 'HH:mm')}</p>
          {!isPast && (
